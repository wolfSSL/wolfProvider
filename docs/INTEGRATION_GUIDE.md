...
| `--enable-pqc` | Enable ML-KEM, ML-DSA and SLH-DSA (adds `--enable-mlkem --enable-mldsa --enable-slhdsa=yes,sha2` to wolfSSL). Requires wolfSSL v5.9.2-stable or later and OpenSSL 3.6 or later. |
| `--enable-slhdsa` | Enable SLH-DSA (FIPS 205) only. The build script enables all 12 parameter sets by default. |
...
| `--enable-mlkem --enable-mldsa --enable-slhdsa=yes,sha2` | ML-KEM, ML-DSA and all 12 SLH-DSA parameter sets (wolfSSL v5.9.2-stable or later). The `build-wolfprovider.sh --enable-pqc` flag sets these automatically. Invoking wolfSSL configure with plain `--enable-slhdsa` enables only the SHAKE parameter sets. |

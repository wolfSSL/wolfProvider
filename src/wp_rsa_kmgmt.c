...
static int wp_rsa_get_params(wp_Rsa* rsa, OSSL_PARAM params[])
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_RSA, "wp_rsa_get_params");
...
static int wp_rsa_has(const wp_Rsa* rsa, int selection)
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_RSA, "wp_rsa_has");
...
static int wp_rsa_match(const wp_Rsa* rsa1, const wp_Rsa* rsa2, int selection)
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_RSA, "wp_rsa_match");
...
static int wp_rsa_validate(const wp_Rsa* rsa, int selection, int checkType)
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_RSA, "wp_rsa_validate");
    ...
    if (ok && d != NULL) {
        ...
        for (i = 0; i < cnt; i++) {
            ...
            if (index < 0) {
                ...
                WOLFPROV_MSG(WP_LOG_COMP_RSA, "Unexpected param %s, skipping.", 
                    p->key);
                continue;
            }
...
static int wp_rsa_import(wp_Rsa* rsa, int selection, const OSSL_PARAM params[])
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_RSA, "wp_rsa_import");
    ...
    int* idx)
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_RSA, "wp_rsa_pss_params_export");
    ...
    unsigned char* data, size_t* idx, int priv)
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_RSA, "wp_rsa_export_keypair");
    ...
    void* cbArg)
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_RSA, "wp_rsa_export");
        ...
        if (rsa != NULL) {
            ...
            for (i = 0; i < WP_RSA_KEYGEN_MAX_RETRY_CNT; i++) {
                ...
                if (rc == PRIME_GEN_E) {
                    ...
                    WOLFPROV_MSG_DEBUG_RETCODE(WP_LOG_LEVEL_DEBUG, "wc_MakeRsaKey", rc);
                    wp_rsa_free(rsa);
                    rsa = NULL;
                    break;
                }
...
static int wp_rsa_gen_set_params(wp_RsaGenCtx* ctx, const OSSL_PARAM params[])
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_RSA, "wp_rsa_gen_set_params");
    ...
    const OSSL_PARAM params[])
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_RSA, "wp_rsa_enc_dec_set_ctx_params");
...
/** First ten bytes shared by the RSA PKCS #1.5 and PSS OIDs. The ninth
 * content byte is deliberately absent to match RSA_PKCS1_5_BYTE or
 * RSA_PKCS1_PSS_BYTE. */
static const unsigned char rsa_pkcs1_oid[] = {
    0x06, 0x09, 0x2a, 0x86, 0x48, 0x86, 0xf7, 0x0d, 0x01, 0x01
};
/** Full RSA PKCS #1 PSS OID. */
static const unsigned char rsa_pss_oid[] = {
...
static int wp_rsa_determine_type(wp_Rsa* rsa, unsigned char* data, word32 len)
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_RSA, "wp_rsa_determine_type");
...
static int wp_rsa_pss_get_params(wp_Rsa* rsa, unsigned char* data, word32 len)
{
    ...
    if (ok && ((idx < 2) || (data[idx - 2] != 0x30))) {
        /* OID is not inside an AlgorithmIdentifier SEQUENCE. */
        ok = 0;
    }
    if (ok) {
        /* Length octet gives the end; a PSS AlgorithmIdentifier is
         * always short form. */
        algIdEnd = idx + data[idx - 1];
        if (algIdEnd > len) {
            ok = 0;
        }
    }
    if (ok) {
        ...
        idx += RSA_PKCS1_OID_SZ + 1;
        if (idx > algIdEnd) {
            ok = 0;
        }
        /* Nothing after the OID means no restrictions. */
        else if (idx < algIdEnd) {
            if (data[idx] != 0x30) {
                ok = 0;
            }
            else {
    ...
    if (ok && haveParams && (idx < algIdEnd) && (data[idx] == 0xa0)) {
        /* Hash algorithm */
    ...
    if (ok && haveParams && (idx < algIdEnd) && (data[idx] == 0xa1)) {
        /* MGF algorithm */
    ...
    if (ok && haveParams && (idx < algIdEnd) && (data[idx] == 0xa2)) {
        /* Salt length. */
    ...
    void* dataCbArg)
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_RSA, "wp_rsa_dec_send_params");
    ...
    OSSL_PASSPHRASE_CALLBACK* pwCb, void* pwCbArg)
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_RSA, WOLFPROV_FUNC_NAME);
 ...
 *
 * @param [in]  rsa        RSA key object. May be NULL when saltLen is an
 *                         explicit length.
 * @param [in]  mdName     Name of digest.
...
/**
 * Encode the AlgorithmIdentifier for an RSA-PSS key in DER.
 *
 * A key with no PSS restrictions encodes as the OID alone.
 *
 * @param [in]  rsa    RSA key object.
 * @param [out] algId  Buffer to hold encoded AlgorithmIdentifier.
 *                     May be NULL and only length calculated.
 * @param [out] len    Length of encoding.
 * @return  1 on success.
 * @return  0 on failure.
 */
static int wp_rsa_pss_encode_key_alg_id(const wp_Rsa* rsa, byte* algId,
    word32* len)
{
    int ok = 1;
    ...
    if (!rsa->pssDefSet) {
        if (algId != NULL) {
            algId[0] = 0x30;
            algId[1] = (byte)sizeof(rsa_pss_oid);
            XMEMCPY(algId + 2, rsa_pss_oid, sizeof(rsa_pss_oid));
        }
        *len = (word32)(2 + sizeof(rsa_pss_oid));
    }
    else {
        ok = wp_rsa_pss_encode_alg_id(rsa, rsa->pssParams.mdName,
            rsa->pssParams.mgfMdName, rsa->pssParams.saltLen, algId, len);
    }
...
static int wp_rsa_encode_spki_size(const wp_Rsa* rsa, size_t* keyLen)
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_RSA, "wp_rsa_encode_spki_size");
    ...
    if (ok && (rsa->type == RSA_FLAG_TYPE_RSASSAPSS)) {
        word32 pssLen = 0;
        ok = wp_rsa_pss_encode_key_alg_id(rsa, NULL, &pssLen);
        if (ok) {
            *keyLen += pssLen;
        }
    }
    ...
    size_t* keyLen)
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_RSA, "wp_rsa_encode_spki");
        ...
        *keyLen = ret;
        if (rsa->type == RSA_FLAG_TYPE_RSASSAPSS) {
            ...
            if (ok) {
                ...
                ok = wp_rsa_pss_encode_key_alg_id(rsa, keyData + 4, &pssLen);
            }
            if (ok) {
                /* Update return length. */
                ret += (int)pssLen - (int)i;
                *keyLen = ret + 4;
...
static int wp_rsa_encode_pub_size(const wp_Rsa* rsa, size_t* keyLen)
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_RSA, "wp_rsa_encode_pub_size");
    ...
    size_t* keyLen)
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_RSA, "wp_rsa_encode_pub");
...
static int wp_rsa_encode_pki_size(const wp_Rsa* rsa, size_t* keyLen, int algoId)
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_RSA, "wp_rsa_encode_pki_size");
    ...
    size_t* keyLen, int algoId)
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_RSA, "wp_rsa_encode_pki");
    ...
    if (ok && rsa->type == RSA_FLAG_TYPE_RSASSAPSS) {
        ...
        if (ok) {
            ...
            /* Encode RSA-PSS Algorithm ID. */
            ok = wp_rsa_pss_encode_key_alg_id(rsa, keyData + 7, &pssLen);
        }
        if (ok) {
            /* Update return length. */
            ret += (int)pssLen - 13;
...
static int wp_rsa_encode_priv_size(const wp_Rsa* rsa, size_t* keyLen)
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_RSA, "wp_rsa_encode_priv_size");
    ...
    size_t* keyLen)
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_RSA, "wp_rsa_encode_priv");
    ...
    OSSL_PASSPHRASE_CALLBACK* pwCb, void* pwCbArg)
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_RSA, "wp_rsa_encode");
    ...
    OSSL_PASSPHRASE_CALLBACK* pwCb, void* pwCbArg)
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_RSA, "wp_rsa_encode_text");

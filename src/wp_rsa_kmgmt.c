...
static wp_Rsa* wp_rsa_base_new(WOLFPROV_CTX* provCtx, int type)
{
    ...
    if (rsa != NULL) {
        ...
        if (rc != 0) {
            WOLFPROV_MSG_DEBUG_RETCODE(WP_LOG_DEBUG, "wc_InitRsaKey", rc);
            ok = 0;
        }
...
static int wp_rsa_validate(const wp_Rsa* rsa, int selection, int checkType)
{
    ...
    if (checkPub && checkPriv) {
        ...
        if (rc != 0) {
            WOLFPROV_MSG_DEBUG_RETCODE(WP_LOG_DEBUG, "wc_CheckRsaKey", rc);
            ok = 0;
        }
    }
    else
    ...
    int selection, const OSSL_PARAM params[], int type)
{
    ...
    if (ctx != NULL) {
        ...
        if (rc != 0) {
            WOLFPROV_MSG_DEBUG_RETCODE(WP_LOG_DEBUG, "wc_InitRng_ex", rc);
            ok = 0;
        }
        if (ok) {
            ctx->provCtx = provCtx;
        ...
        if (rsa != NULL) {
            ...
            for (i = 0; i < WP_RSA_KEYGEN_MAX_RETRY_CNT; i++) {
                ...
                if (rc == PRIME_GEN_E) {
                    ...
                    WOLFPROV_MSG_DEBUG_RETCODE(WP_LOG_DEBUG, "wc_MakeRsaKey", rc);
                    wp_rsa_free(rsa);
                    rsa = NULL;
                    break;
                }
    ...
    word32 oidLen, word32* offset)
{
    ...
    WOLFPROV_ENTER_SILENT(WP_LOG_RSA, WOLFPROV_FUNC_NAME);
    ...
    OSSL_PASSPHRASE_CALLBACK* pwCb, void* pwCbArg)
{
    ...
    WOLFPROV_ENTER(WP_LOG_RSA, WOLFPROV_FUNC_NAME);

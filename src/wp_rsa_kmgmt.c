 ...
 * @param [out] ossl_digest Corresponding OpenSSL digest name.
 * @return  1 on success.
 * @return  0 on failure.
 */
        ...
        const char **ossl_digest)
{
    ...
    case WC_HASH_TYPE_MD5:
        *ossl_digest = OSSL_DIGEST_NAME_MD5;
        break;

    case WC_HASH_TYPE_SHA:
        *ossl_digest = OSSL_DIGEST_NAME_SHA1;
        break;

    case WC_HASH_TYPE_SHA256:
        *ossl_digest = OSSL_DIGEST_NAME_SHA2_256;
        break;

    case WC_HASH_TYPE_SHA384:
        *ossl_digest = OSSL_DIGEST_NAME_SHA2_384;
        break;

    case WC_HASH_TYPE_SHA512:
        *ossl_digest = OSSL_DIGEST_NAME_SHA2_512;
        break;
...
static int wp_rsa_get_params_pss(wp_RsaPssParams* pss,  OSSL_PARAM params[])
{
    ...
    if (pss->hashType != WP_RSA_PSS_DIGEST_DEF) {
        ...
        if ((p != NULL) && wp_digest_to_ossl_digest(pss->hashType, &ossl_digest)
                && !OSSL_PARAM_set_utf8_string(p, ossl_digest)) {
            ok = 0;
        }
    }

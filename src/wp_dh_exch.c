...
static int wp_dh_init(wp_DhCtx* ctx, wp_Dh* dh, const OSSL_PARAM params[])
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_DH, "wp_dh_init");
    ...
    size_t* keyLen, size_t keySize, unsigned char* sec, size_t secLen)
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_DH, "wp_dh_kdf_derive");
    ...
    if (ok) {
        ...
        if (rc != 0) {
            WOLFPROV_MSG_DEBUG_RETCODE(WP_LOG_LEVEL_DEBUG, "wc_X963_KDF", rc);
            ok = 0;
        }
        else {
            *keyLen = ctx->keyLen;
    ...
    size_t* secLen, size_t maxLen)
{
    ...
    if (ok) {
        ...
        PRIVATE_KEY_LOCK();
        if (rc != 0) {
            WOLFPROV_MSG_DEBUG_RETCODE(WP_LOG_LEVEL_DEBUG, "wc_DhAgree", rc);
            ok = 0;
        }
        else {
    ...
    size_t* secLen, size_t secSize)
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_DH, "wp_dh_derive");
...
static int wp_dh_set_peer(wp_DhCtx* ctx, wp_Dh* peer)
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_DH, "wp_dh_set_peer");
...
static int wp_dh_set_param_kdf(wp_DhCtx* ctx, const OSSL_PARAM params[])
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_DH, "wp_dh_set_param_kdf");
...
static int wp_dh_set_param_kdf_digest(wp_DhCtx* ctx, const OSSL_PARAM params[])
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_DH, "wp_dh_set_param_kdf_digest");
...
static int wp_dh_set_ctx_params(wp_DhCtx* ctx, const OSSL_PARAM params[])
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_DH, "wp_dh_set_ctx_params");
...
static int wp_dh_get_params_kdf(wp_DhCtx* ctx, OSSL_PARAM params[])
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_DH, "wp_dh_get_params_kdf");
...
static int wp_dh_get_ctx_params(wp_DhCtx* ctx, OSSL_PARAM params[])
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_DH, "wp_dh_get_ctx_params");

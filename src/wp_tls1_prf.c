    ...
    size_t keyLen, const OSSL_PARAM params[])
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_TLS1_PRF, "wp_kdf_tls1_prf_derive");
    ...
    if (ok) {
        ...
        if (ctx->mdType == WC_HASH_TYPE_MD5_SHA) {
            ...
            PRIVATE_KEY_LOCK();
            if (rc != 0) {
                WOLFPROV_MSG_DEBUG_RETCODE(WP_LOG_LEVEL_DEBUG, "wc_PRF_TLSv1", rc);
                ok = 0;
            }
        }
        else {
            ...
            PRIVATE_KEY_LOCK();
            if (rc != 0) {
                WOLFPROV_MSG_DEBUG_RETCODE(WP_LOG_LEVEL_DEBUG, "wc_PRF_TLS", rc);
                ok = 0;
            }
        }
    }
    ...
    const OSSL_PARAM params[])
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_TLS1_PRF, "wp_kdf_tls1_prf_get_seed");
    ...
    const OSSL_PARAM params[])
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_TLS1_PRF, "wp_kdf_tls1_prf_set_ctx_params");
    ...
    OSSL_PARAM params[])
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_TLS1_PRF, "wp_kdf_tls1_prf_get_ctx_params");

...
static int wp_ecdh_init(wp_EcdhCtx* ctx, wp_Ecc* ecc, const OSSL_PARAM params[])
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_ECDH, "wp_ecdh_init");
    ...
    size_t* secLen)
{
    ...
    if ((ok = wp_ecc_check_usage(ctx->key))) {
        ...
        PRIVATE_KEY_LOCK();
        if (rc != 0) {
            WOLFPROV_MSG_DEBUG_RETCODE(WP_LOG_LEVEL_DEBUG, "wc_ecc_shared_secret", rc);
            ok = 0;
        }
    }
    if (ok) {
...
static int wp_ecdh_set_param_kdf(wp_EcdhCtx* ctx, const OSSL_PARAM params[])
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_ECDH, "wp_ecdh_set_param_kdf");
    ...
    const OSSL_PARAM params[])
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_ECDH, "wp_ecdh_set_param_kdf_digest");
...
static int wp_ecdh_set_ctx_params(wp_EcdhCtx* ctx, const OSSL_PARAM params[])
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_ECDH, "wp_ecdh_set_ctx_params");
...
static int wp_ecdh_get_params_kdf(wp_EcdhCtx* ctx, OSSL_PARAM params[])
{
     ...
     WOLFPROV_ENTER(WP_LOG_COMP_ECDH, "wp_ecdh_get_params_kdf");
...
static int wp_ecdh_get_ctx_params(wp_EcdhCtx* ctx, OSSL_PARAM params[])
{
     ...
     WOLFPROV_ENTER(WP_LOG_COMP_ECDH, "wp_ecdh_get_ctx_params");

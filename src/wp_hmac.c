...
static wp_HmacCtx* wp_hmac_new(WOLFPROV_CTX* provCtx)
{
    ...
    if (macCtx != NULL) {
        ...
        if (rc != 0) {
            WOLFPROV_MSG_DEBUG_RETCODE(WP_LOG_LEVEL_DEBUG, "wc_HmacInit", rc);
            OPENSSL_free(macCtx);
            macCtx = NULL;
        }
    }
    ...
    size_t keyLen, const OSSL_PARAM params[])
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_MAC, "wp_hmac_init");
    ...
    size_t outSize)
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_MAC, "wp_hmac_final");
...
static int wp_hmac_get_ctx_params(wp_HmacCtx* macCtx, OSSL_PARAM params[])
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_MAC, "wp_hmac_get_ctx_params");
...
static int wp_hmac_set_ctx_params(wp_HmacCtx* macCtx, const OSSL_PARAM params[])
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_MAC, "wp_hmac_set_ctx_params");

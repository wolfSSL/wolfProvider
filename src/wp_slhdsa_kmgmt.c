...
int wp_slhdsa_up_ref(wp_SlhDsa* slhdsa)
{
    ...
    rc = wc_LockMutex(&slhdsa->mutex);
    if (rc < 0) {
        ok = 0;
    }
    if (ok) {
        slhdsa->refCnt++;
        wc_UnLockMutex(&slhdsa->mutex);
    }
...
{
    const wp_SlhDsa* slhdsa = *pSlhDsa;
    (void)size;
    *pSlhDsa = NULL;
    return slhdsa;
}
    ...
    const OSSL_PARAM params[])
{
    ...
    if (ok && ((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0)) {
        ...
        if (ok && (privData != NULL)) {
            ...
            if (ok) {
                /* Unlike ML-DSA, the 4n FIPS 205 private key ends with the
                 * public key, so importing it yields both halves. */
                slhdsa->hasPriv = 1;
                slhdsa->hasPub = 1;
            }
        }
...
#ifdef WP_HAVE_SLHDSA_PRIVATE
    /* Catches a mismatched pub/priv pair that the imports accept separately.
     * Only when both were supplied: CheckKey rebuilds the public root, which
     * is slow for the 's' parameter sets. */
    if (ok && (privData != NULL) && (pubData != NULL)) {
...
static const OSSL_PARAM wp_slhdsa_key_params[] = {
    /* 0: none */
    OSSL_PARAM_END,
...
static const OSSL_PARAM* wp_slhdsa_key_types(int selection)
{
    ...
    (void)extra;
#endif
    return &wp_slhdsa_key_params[idx];
}
    ...
    const OSSL_PARAM params[])
{
    ...
    if (p != NULL) {
        ...
        /* A short seed would silently fall back to RNG keygen, breaking the
         * caller's reproducibility contract. Require exactly 3n. */
        if (ctx->seedLen != (size_t)((3 * ctx->data->pubKeySize) / 2)) {
            WOLFPROV_LEAVE(WP_LOG_COMP_PQC, __FILE__ ":" WOLFPROV_STRINGIZE(__LINE__), 0);
            return 0;
        }
    }
...
static int wp_slhdsa_pub_decode(const byte* input, word32* inOutIdx, void* key,
    word32 inSz)
{
    return wc_SlhDsaKey_PublicKeyDecode(input, inOutIdx, (SlhDsaKey*)key, inSz);
}
...
const OSSL_DISPATCH wp_##alg##_##fmt##_##enc##_encoder_functions[] = {         \
    ...
    { OSSL_FUNC_ENCODER_IMPORT_OBJECT, (DFUNC)wp_slhdsa_import              }, \

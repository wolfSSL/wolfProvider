...
static int wp_mp_rand(mp_int* a, int digits, WC_RNG* rng)
{
    ...
    cnt = (size_t)digits * sizeof(mp_digit);
    buf = (unsigned char*)OPENSSL_malloc(cnt);
    if (buf == NULL) {
        return MEMORY_E;
    }

    rc = wc_RNG_GenerateBlock(rng, buf, (word32)cnt);
    if (rc == 0) {
        rc = mp_read_unsigned_bin(a, buf, (int)cnt);
    }
    OPENSSL_clear_free(buf, cnt);
...
static int wp_rsasve_gen_rand_bytes(wp_RsaKemCtx* ctx, unsigned char* out)
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_RSA, "wp_rsasve_gen_rand_bytes");
    ...
    if (ok) {
        ...
        if (rc != 0) {
            WOLFPROV_MSG_DEBUG_RETCODE(WP_LOG_LEVEL_DEBUG, "mp_sub_d", rc);
            ok = 0;
        }
    }
    while (ok) {
        ...
        if (rc != 0) {
            WOLFPROV_MSG_DEBUG_RETCODE(WP_LOG_LEVEL_DEBUG, "wp_mp_rand", rc);
            ok = 0;
        }
    ...
    if (ok) {
        ...
        rc = mp_add_d(&r, 2, &r);
        if (rc != 0) {
            WOLFPROV_MSG_DEBUG_RETCODE(WP_LOG_LEVEL_DEBUG, "mp_add_d", rc);
            ok = 0;
        }
    }
    if (ok) {
    ...
    size_t* outLen, const unsigned char* in, size_t inLen)
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_RSA, "wp_rsasve_recover");
    ...
    const OSSL_PARAM params[])
{
    ...
    WOLFPROV_ENTER(WP_LOG_COMP_RSA, "wp_rsakem_set_ctx_params");

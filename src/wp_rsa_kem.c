...
static int wp_rsasve_gen_rand_bytes(wp_RsaKemCtx* ctx, unsigned char* out)
{
    ...
    if (ok) {
        ...
        if (rc != 0) {
            WOLFPROV_MSG_DEBUG(WP_LOG_DEBUG, "mp_sub_d failed with rc=%d", rc);
            ok = 0;
        }
    }
    while (ok) {
        ...
        if (rc != 0) {
            WOLFPROV_MSG_DEBUG(WP_LOG_DEBUG, "wp_mp_rand failed with rc=%d", rc);
            ok = 0;
        }
    ...
    if (ok) {
        ...
        rc = mp_add_d(&r, 2, &r);
        if (rc != 0) {
            WOLFPROV_MSG_DEBUG(WP_LOG_DEBUG, "mp_add_d failed with rc=%d", rc);
            ok = 0;
        }
    }
    if (ok) {

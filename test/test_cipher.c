...
/* CTS of an input of exactly one block must use the cached IV when the key and
 * IV are set by separate init calls, in either order. */
int test_aes128_cts_one_block_split_init(void *data)
{
    ...
    for (keyFirst = 0; err == 0 && keyFirst < 2; keyFirst++) {
        ...
        if (err == 0 && (outlen + finallen != (int)sizeof(msg))) {
            PRINT_MSG("One block split init CTS encrypt length mismatch");
            err = 1;
        }
        if (err == 0 && memcmp(enc, ref, sizeof(msg)) != 0) {
            PRINT_MSG("One block split init CTS ciphertext differs from "
                "OpenSSL");
            err = 1;
        }
        EVP_CIPHER_CTX_free(ctx);
        ...
        if (err == 0 && (outlen + finallen != (int)sizeof(msg))) {
            PRINT_MSG("One block split init CTS decrypt length mismatch");
            err = 1;
        }
        if (err == 0 && memcmp(dec, msg, sizeof(msg)) != 0) {
            PRINT_MSG("One block split init CTS round trip failed");
            err = 1;
        }
        for (i = sizeof(msg); (err == 0) && (i < sizeof(dec)); i++) {
            if (dec[i] != 0xa5) {
                PRINT_MSG("One block split init CTS decrypt wrote past the "
                    "input length");
                err = 1;
            }
        }

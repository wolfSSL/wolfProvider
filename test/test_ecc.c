...
#ifdef WP_HAVE_X448
/* RFC 7748 Section 6.2 X448 test vectors (from OpenSSL evppkey_ecx.txt). */
static const unsigned char x448_alice_priv[] = {
...
static const unsigned char x448_bob_priv[] = {
...
static const unsigned char x448_shared_secret[] = {
...
/*
 * X448 known-answer ECDH test using RFC 7748 Section 6.2 vectors.
 * Validates endian-swap logic in wp_x448_derive by comparing the
 * derived shared secret against the expected RFC output.
 */
int test_ecdh_x448_vector(void *data)
{
    ...
    PRINT_MSG("X448 ECDH known-answer test (RFC 7748 Sec 6.2)");

    keyA = EVP_PKEY_new_raw_private_key_ex(wpLibCtx, "X448", NULL,
        x448_alice_priv, sizeof(x448_alice_priv));
    if (keyA == NULL) {
        PRINT_ERR_MSG("Failed to import X448 Alice private key");
        err = 1;
    }
    if (err == 0) {
        keyB = EVP_PKEY_new_raw_private_key_ex(wpLibCtx, "X448", NULL,
            x448_bob_priv, sizeof(x448_bob_priv));
        if (keyB == NULL) {
            PRINT_ERR_MSG("Failed to import X448 Bob private key");
            err = 1;
        }
    }
    if (err == 0) {
        err = test_ecdh_derive(keyA, keyB, &secret, 56);
    }
    if (err == 0) {
        if (memcmp(secret, x448_shared_secret, 56) != 0) {
            PRINT_ERR_MSG("X448 shared secret does not match expected");
            PRINT_BUFFER("Got", secret, 56);
            PRINT_BUFFER("Expected",
                (unsigned char *)x448_shared_secret, 56);
            err = 1;
        }
    }

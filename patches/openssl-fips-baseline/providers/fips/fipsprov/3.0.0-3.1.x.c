 ...
 *
 * Modified for wolfProvider FIPS Baseline - Minimal FIPS provider for 3.0.0-3.1.x
 */
...
static const OSSL_PARAM fips_param_types[] = {
    ...
    OSSL_PARAM_DEFN(OSSL_PROV_FIPS_PARAM_SECURITY_CHECKS, OSSL_PARAM_INTEGER, NULL, 0),
    OSSL_PARAM_DEFN(OSSL_PROV_FIPS_PARAM_TLS1_PRF_EMS_CHECK, OSSL_PARAM_INTEGER, NULL, 0),
    OSSL_PARAM_DEFN(OSSL_PROV_FIPS_PARAM_DRBG_TRUNC_DIGEST, OSSL_PARAM_INTEGER, NULL, 0),
    OSSL_PARAM_END
};
...
static int fips_get_params(void *provctx, OSSL_PARAM params[])
{
    ...
    if (p != NULL && !OSSL_PARAM_set_int(p, ossl_prov_is_running()))
        return 0;
    p = OSSL_PARAM_locate(params, OSSL_PROV_FIPS_PARAM_SECURITY_CHECKS);
    if (p != NULL && !OSSL_PARAM_set_int(p, fgbl->fips_security_checks.enabled))
        return 0;
    p = OSSL_PARAM_locate(params, OSSL_PROV_FIPS_PARAM_TLS1_PRF_EMS_CHECK);
    if (p != NULL && !OSSL_PARAM_set_int(p, fgbl->fips_tls1_prf_ems_check.enabled))
        return 0;
    p = OSSL_PARAM_locate(params, OSSL_PROV_FIPS_PARAM_DRBG_TRUNC_DIGEST);
    if (p != NULL && !OSSL_PARAM_set_int(p, fgbl->fips_restricted_drgb_digests.enabled))
        return 0;
    return 1;
}
...
static int fips_get_params_from_core(FIPS_GLOBAL *fgbl)
{
    ...
    * Parameters to retrieve from the core provider.
    * NOTE: inside core_get_params() these will be loaded from config items
    * stored inside prov->parameters (except OSSL_PROV_PARAM_CORE_VERSION).
    */
    ...
    *p++ = OSSL_PARAM_construct_utf8_ptr(
            ...
            (char **)&fgbl->fips_security_checks.option,
            sizeof(fgbl->fips_security_checks.option));
    *p++ = OSSL_PARAM_construct_utf8_ptr(
            OSSL_PROV_FIPS_PARAM_TLS1_PRF_EMS_CHECK,
            (char **)&fgbl->fips_tls1_prf_ems_check.option,
            sizeof(fgbl->fips_tls1_prf_ems_check.option));
    *p++ = OSSL_PARAM_construct_utf8_ptr(
            OSSL_PROV_FIPS_PARAM_DRBG_TRUNC_DIGEST,
            (char **)&fgbl->fips_restricted_drgb_digests.option,
            sizeof(fgbl->fips_restricted_drgb_digests.option));
 ...
 * 3.0.0-3.1.x version - uses simpler structure without composite signatures
 */

/* FIPS Baseline Digests - SHA family only */
...
#define set_func(c, f) if (c == NULL) c = f; else if (c != f) return 0;
    ...
    if (fgbl->fips_security_checks.option != NULL
        && strcmp(fgbl->fips_security_checks.option, "0") == 0)
        fgbl->fips_security_checks.enabled = 0;

    /* Enable TLS1 PRF EMS check if requested */
    if (fgbl->fips_tls1_prf_ems_check.option != NULL
        && strcmp(fgbl->fips_tls1_prf_ems_check.option, "1") == 0)
        fgbl->fips_tls1_prf_ems_check.enabled = 1;

    /* Enable restricted DRBG digests if requested */
    if (fgbl->fips_restricted_drgb_digests.option != NULL
        && strcmp(fgbl->fips_restricted_drgb_digests.option, "1") == 0)
        fgbl->fips_restricted_drgb_digests.enabled = 1;
...
int ossl_fips_config_security_checks(OSSL_LIB_CTX *libctx)
{
    ...
    return fgbl->fips_security_checks.enabled;
}
...
int ossl_fips_config_tls1_prf_ems_check(OSSL_LIB_CTX *libctx)
{
    ...
    return fgbl->fips_tls1_prf_ems_check.enabled;
}

int ossl_fips_config_restricted_drgb_digests(OSSL_LIB_CTX *libctx)
{
    FIPS_GLOBAL *fgbl = ossl_lib_ctx_get_data(libctx,
                                              OSSL_LIB_CTX_FIPS_PROV_INDEX);

    return fgbl->fips_restricted_drgb_digests.enabled;
}
